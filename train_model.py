...
class DatasetIngestionService:
    ...
    def load_enron(self, filepath: str) -> pd.DataFrame:
        ...
        if subject_col in df.columns and message_col in df.columns:
            ...
            print(
                "[-] Critical Error: Could not find a text or message column in CSV.",
                file=sys.stderr,
            )
            return pd.DataFrame(columns=["text", "label"])
        ...
        if label_col in df.columns:
            ...
            print(
                "[-] Critical Error: Could not find a label column in CSV.",
                file=sys.stderr,
            )
            return pd.DataFrame(columns=["text", "label"])
    ...
    def build_unified_dataset(
        self, enron_path: str, sa_spam: str, sa_ham: str
    ) -> pd.DataFrame:
        ...
        if enron_df.empty and sa_df.empty:
            raise ValueError(
                "Data pipeline termination: All incoming ingestion vectors are completely empty."
            )
        ...
        if df["label"].nunique() < 2:
            raise ValueError(
                f"Data stratification error: Found fewer than two target tracking classes. Current distribution:\n{df['label'].value_counts()}"
            )
        ...
        if df["label"].nunique() < 2:
            raise ValueError(
                "Data stratification error: Text cleaning stripped away all remaining instances of a class."
            )

        print(
            f"[+] Combined Clean Dataset Distribution:\n{df['label'].value_counts().to_string()}"
        )
        return df
...
class ModelTrainingPipeline:
    ...
    def execute(self, data_frame: pd.DataFrame):
        ...
        print(
            f"[*] Dispatching Calibrated LinearSVC Engine [Training Size: {X_train.shape[0]} arrays]..."
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"\n[-] Operational Failure in Data Pipeline execution sequence: {error}",
            file=sys.stderr,
        )
        sys.exit(1)
