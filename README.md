...
**Scan. Classify. Audit.**
A machine-learning-powered email spam detector with a full dashboard for browsing, editing, and auditing every scan.
...
SpamShield AI scans email content and classifies it as **SPAM** or **HAM** using a machine learning model, then logs every result to a searchable, editable dashboard.

The app is split into two independent services:

- 🌐 **Flask web app** — handles the UI, database records, and stats
- 🧠 **FastAPI inference service** — runs the actual classification model
...
| Input sanitization | Bleach + Regex | Strips HTML/scripts and cleans whitespace before storage/processing, to prevent XSS |
| Data store | MongoDB | Stores each scanned email's sanitized text, label, confidence score, and timestamp |

**Request flow:**
`User submits email text` → `Flask sanitizes input` → `Flask calls FastAPI inference endpoint` → `FastAPI returns label + confidence` → `Flask saves result to MongoDB and renders it`

---

## ⚙️ CI/CD Pipeline

GitHub Actions (`.github/workflows/ci-cd-pipeline.yml`) runs automatically on every push/PR and checks:
...
> ⚠️ **Don't use the example secret key in production.** Generate your own with:
> ```bash
...
Confirm it's up by visiting `http://localhost:8000/docs` to see the auto-generated Swagger UI for the `/api/v1/predict` endpoint.
...
```
This spins up Flask, FastAPI, and MongoDB together. Check the compose file for the exact exposed ports.
...
Made with 🛡️ to keep inboxes clean.
